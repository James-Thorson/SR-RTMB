...
run_occupancy_par <- function(nsim, R, T, psi_true, p_true, seed,
  ...
  list(
    model = "occupancy", estimates = estimates, times = times,
...
run_nmixture_par <- function(nsim, R, T, lambda_true, p_true, seed,
  ...
  conv_rate <- mean(sapply(raw, function(x) x$jags_converged), na.rm = TRUE)
  list(
    model = "nmixture", estimates = estimates, times = times,
...
run_dailmadsen_par <- function(nsim, M, T, lambda_true, gamma_true, omega_true,
  ...
  jags_raw <- mclapply(datasets, function(y) {
    ...
    list(
      estimates = out$estimates, jags_converged = out$jags_converged,
      time_jags = t["elapsed"]
    )
  }, mc.cores = mc.cores)
...
res_occ <- run_occupancy_par(
  nsim = nsim, R = occ_R, T = occ_T,
  psi_true = occ_psi, p_true = occ_p, seed = occ_seed,
...
res_nmix <- run_nmixture_par(
  nsim = nsim, R = nmix_R, T = nmix_T,
  lambda_true = nmix_lambda, p_true = nmix_p, seed = nmix_seed,
  n.chains = jags_chains, n.iter = jags_iter,
  n.burnin = jags_burnin, n.thin = jags_thin,
  mc.cores = mc.cores
)
...
res_dm <- run_dailmadsen_par(
  nsim = nsim, M = dm_M, T = dm_T,
  lambda_true = dm_lambda, gamma_true = dm_gamma,
  omega_true = dm_omega, p_true = dm_p, seed = dm_seed,
...
if (!is.na(header)) {
  ...
  readme[start + 1] <- paste0(
    "| N-mixture | ", timing$RTMB_mean_s[2],
  ...
  readme[end] <- paste0(
    "| Dail-Madsen | ", timing$RTMB_mean_s[3],
    " | ", timing$unmarked_mean_s[3],
    " | ", timing$JAGS_mean_s[3],
    " | ", timing$speedup_vs_unm[3], "x",
    " | ", timing$speedup_vs_jags[3], "x |"
  )
  # Update convergence rate table
  conv_header <- grep("\\| Model \\| Convergence", readme)[1]
  if (!is.na(conv_header)) {
    readme[conv_header + 2L] <- paste0("| Occupancy | ", round(res_occ$jags_conv_rate, 2), " |")
    readme[conv_header + 3L] <- paste0("| N-mixture | ", round(res_nmix$jags_conv_rate, 2), " |")
    readme[conv_header + 4L] <- paste0("| Dail-Madsen | ", round(res_dm$jags_conv_rate, 2), " |")
  }
...
plot_hist <- function(x, truth, col, main, xlab = "Estimate") {
  hist(x,
    col = col, border = "white", main = main,
...
plot_hist(
  est_occ$psi_rtmb, res_occ$truth["psi"], col_rtmb,
  expression(bold(paste("RTMB  ", psi)))
)
plot_hist(
  est_occ$p_rtmb, res_occ$truth["p"], col_rtmb,
  expression(bold(paste("RTMB  ", italic(p))))
)
plot_hist(
  est_occ$psi_unm, res_occ$truth["psi"], col_unm,
  expression(bold(paste("unmarked  ", psi)))
)
plot_hist(
  est_occ$p_unm, res_occ$truth["p"], col_unm,
  expression(bold(paste("unmarked  ", italic(p))))
)
plot_hist(
  est_occ$psi_jags, res_occ$truth["psi"], col_jags,
  expression(bold(paste("JAGS  ", psi)))
)
plot_hist(
  est_occ$p_jags, res_occ$truth["p"], col_jags,
...
plot_hist(
  est_nmix$lambda_rtmb, res_nmix$truth["lambda"], col_rtmb,
  expression(bold(paste("RTMB  ", lambda)))
)
plot_hist(
  est_nmix$p_rtmb, res_nmix$truth["p"], col_rtmb,
  expression(bold(paste("RTMB  ", italic(p))))
)
plot_hist(
  est_nmix$lambda_unm, res_nmix$truth["lambda"], col_unm,
  expression(bold(paste("unmarked  ", lambda)))
)
plot_hist(
  est_nmix$p_unm, res_nmix$truth["p"], col_unm,
  expression(bold(paste("unmarked  ", italic(p))))
)
plot_hist(
  est_nmix$lambda_jags, res_nmix$truth["lambda"], col_jags,
  expression(bold(paste("JAGS  ", lambda)))
)
plot_hist(
  est_nmix$p_jags, res_nmix$truth["p"], col_jags,
  expression(bold(paste("JAGS  ", italic(p))))
)
par(op)
dev.off()
...
# Row 1: RTMB
plot_hist(
  est_dm_rtmb$lambda, res_dm$truth["lambda"], col_rtmb,
  expression(bold(paste("RTMB  ", lambda)))
)
plot_hist(
  est_dm_rtmb$gamma, res_dm$truth["gamma"], col_rtmb,
  expression(bold(paste("RTMB  ", gamma)))
)
plot_hist(
  est_dm_rtmb$omega, res_dm$truth["omega"], col_rtmb,
  expression(bold(paste("RTMB  ", omega)))
)
plot_hist(
  est_dm_rtmb$p, res_dm$truth["p"], col_rtmb,
  expression(bold(paste("RTMB  ", italic(p))))
)
# Row 2: unmarked
plot_hist(
  est_dm_unm$lambda, res_dm$truth["lambda"], col_unm,
  expression(bold(paste("unmarked  ", lambda)))
)
plot_hist(
  est_dm_unm$gamma, res_dm$truth["gamma"], col_unm,
  expression(bold(paste("unmarked  ", gamma)))
)
plot_hist(
  est_dm_unm$omega, res_dm$truth["omega"], col_unm,
  expression(bold(paste("unmarked  ", omega)))
)
plot_hist(
  est_dm_unm$p, res_dm$truth["p"], col_unm,
  expression(bold(paste("unmarked  ", italic(p))))
)
# Row 3: JAGS
plot_hist(
  est_dm_jags$lambda, res_dm$truth["lambda"], col_jags,
  expression(bold(paste("JAGS  ", lambda)))
)
plot_hist(
  est_dm_jags$gamma, res_dm$truth["gamma"], col_jags,
  expression(bold(paste("JAGS  ", gamma)))
)
plot_hist(
  est_dm_jags$omega, res_dm$truth["omega"], col_jags,
  expression(bold(paste("JAGS  ", omega)))
)
plot_hist(
  est_dm_jags$p, res_dm$truth["p"], col_jags,
  expression(bold(paste("JAGS  ", italic(p))))
)
par(op)
dev.off()
